...
router.patch('/address', authMiddleware, async (req, res) => {
  const { subscription_id, shipping_address_1, shipping_suburb, shipping_state, shipping_postcode } = req.body;
  ...
  try {
    const result = await db.query(
      `UPDATE subscriptions
       ...
       RETURNING *`,
      [shipping_address_1, shipping_suburb, shipping_state, shipping_postcode, subscription_id, req.customer.id]
    );
...
router.patch('/coffee', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    if (productRow.rows.length === 0) return res.status(404).json({ error: 'Product not found' });
